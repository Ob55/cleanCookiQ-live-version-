-- ============================================================
-- Closes methodology gap (d): IRR fit in deal matching
--
-- The funder deal-flow view did not expose enough inputs to
-- compute a per-project IRR client-side. This migration adds
-- the institution-level fuel-spend and stored savings figures
-- so the funder portal can run tcoModel() per row and replace
-- the placeholder IRR fit (1.0 / 0.5) with a real signal.
--
-- We DROP and recreate the view rather than CREATE OR REPLACE
-- because the new columns are inserted mid-list — Postgres
-- rejects column-order changes on CREATE OR REPLACE VIEW
-- with ERROR 42P16. CASCADE is safe here: nothing in the DB
-- depends on this view (it's consumed by the frontend only).
-- ============================================================
